...
```
tool_connections/
  jira-api-token.md             ← Jira: fetch, search, create, update issues
  github-api-token.md           ← GitHub: repos, PRs, issues, code search
  confluence-api-token.md       ← Confluence: search and read internal docs
  grafana-sso-session.md        ← Grafana: extract PromQL, find dashboards
...
verified_connections.example.md   ← master catalog of all available tool connections
verified_connections.md           ← your active connections (generated by setup, gitignored)
...
2. Ask your agent: *"Read SETUP.md and set up my tool connections"*
3. Your agent generates `verified_connections.md` — load it at session start

**Contributor** — you want to add a new tool connection and open a PR:
...
**Owner / reviewer** — you also use and contribute, plus you review incoming PRs:
1. Review guide lives in `scripts/review-community-pr.md` (gitignored — owner's local copy only)
2. After approval: promote solid community files to `tool_connections/` via the promotion flow in that guide
